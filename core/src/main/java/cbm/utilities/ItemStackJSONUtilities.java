...
public class ItemStackJSONUtilities {
			...
			if (className != null) {
				if (className.equalsIgnoreCase("ItemMeta")) {
					try {
						...
						return MethodReflection.callStaticMethod(classy, "deserialize", map);
					...
					try {
						return MethodReflection.callStaticMethod(stringToClass.get(className), "deserialize", map);
